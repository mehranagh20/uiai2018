...
def init_players():
    ...
    players = [Player(name="player_1", first_pos=Pos(-6.5, .6)),
               ...
               Player(name="player_3", first_pos=Pos(-6, 3)),
...
def do_turn(game):
    my_team = game.getMyTeam()
    for i in range(4):
        p = my_team.getPlayer(i)
        print(p.getId(), p.getPosition())
    act = Triple()
    act.setPlayerID(2)
    act.setAngle(15)
    act.setPower(100)
    return act
    # TODO score if you can
    # TODO Defend better if you can

# def do_turn(game):
#     act = Triple()
#     '''
#     Write your code here
#     At the end you have to set 3kparameter:
